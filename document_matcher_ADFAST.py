...
class DocumentMatcherGUI:
    ...
    # To avoid language barrier between English and French
    ...
    def extract_po_number(self, text: str) -> str:
        """Extract PO number from either document type."""
        # ADFAST: Référence du client (PO): XXXX
        m = re.search(r'R[ée]f[ée]rence du client\s*\(PO\)[:\s]+(\d+)', text, re.IGNORECASE)
        if m:
            return m.group(1).strip()

        # The PO box is in the top-right; we look for the label then the next number.
        m = re.search(
    ...
    def parse_dalmen_order(self, text: str) -> OrderDocument:
        ...
        while i < len(raw_lines):
            ...
            if re.match(r'^\d{4}-\s', line, re.IGNORECASE):
                if i + 1 < len(raw_lines):
                    next_line = raw_lines[i + 1].strip()
                    if re.match(r'^\d+ml', next_line, re.IGNORECASE):
                        ml_match = re.match(r'^(\d+ml[\w/,]*)', next_line, re.IGNORECASE)
                        if ml_match:
                            line = line.replace('4551-', '4551-' + ml_match.group(1) + ' ')
        ...
        for line in merged_lines:
            ...
            if not code_match:
                partial = re.match(r'^(\d{4}-)', line, re.IGNORECASE)
                if partial:
                    ml_match = re.search(r'(\d+\s*ml)', line, re.IGNORECASE)
                    if ml_match:
                        raw_code = partial.group(1) + ml_match.group(1).replace(' ', '')
                    else:
                        continue
                else:
                    continue
            ...
            # Note: Dalmen unit prices can have 3 decimal places (e.g. 119.400)
    ...
    def parse_adfast_confirmation(self, text: str) -> OrderDocument:
        ...
        while i < len(lines):
            ...
            # Match ADFAST product codes between order and confirmation
            code_match = re.match(r'^(\d{4}-\d+\s*ml[/\w]*)', line, re.IGNORECASE)
            if code_match:
                raw_code = code_match.group(1)
                product_code = self.normalize_code(raw_code)
                ...
                # Total (line total) is the LAST number in the block.
                price = 0.0
                unit_price = 0.0
                ...
                unit_price = float_nums[-2] if len(float_nums) >= 2 else (float_nums[-1] if float_nums else 0.0)
                price = float_nums[-1] if float_nums else 0.0
        ...
        # Total — SOUS-TOTAL (pre-tax, matches Dalmen's total)
    ...
    def match_documents(self, dalmen: OrderDocument, adfast: OrderDocument) -> Dict:
        ...
        # Per-line matching (code + color + price)
        ...
        for d_item in dalmen.line_items:
            ...
            for idx, a_item in enumerate(adfast.line_items):
                ...
                # Both docs have unit prices; Dalmen's Prix unitaire vs ADFAST's Prix
                d_unit = d_item.price 
                a_unit = a_item.price 
            ...
            # PO is global. We pass if (po_match + best_score) >= 3.
        ...
        overall_match = matched_items == total_items  # all items must pass
    ...
    def display_result(self, result):
        ...
        # Top row: icon + verdict + confidence 
        ...
        stat_box(stats, "Item Code",     f"{result['matched_items']} / {result['total_items']}", 1)
